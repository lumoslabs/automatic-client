...
describe Automatic::Models::Webhook::NotificationSpeeding do
  let(:notification_speeding_file) do
    File.read(File.expand_path('../../../../../data/models/webhook/notification_speeding.json', __FILE__))
  end

  let(:notification_speeding) do
    MultiJson.load(notification_speeding_file)
  end

  let(:attributes) { notification_speeding }
  ...
  context "with all values" do
    ...
    it "returns the #type" do
      expect(subject.type).to eq('notification:speeding')
    end

    it "returns the #speed_mph" do
      expect(subject.speed_mph).to eq(83)
    end
